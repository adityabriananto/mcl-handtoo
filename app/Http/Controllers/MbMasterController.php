...
class MbMasterController extends Controller
{
    ...
    {
        // 1. Identifikasi Barcode dan Status Awal
        $barcode = $mbMaster->manufacture_barcode;
        $oldStatus = (int) $mbMaster->is_disabled;

        // 2. Cek apakah ini Multi-Brand (Cek di master ada berapa brand untuk barcode ini)
        $brandCount = MbMaster::where('manufacture_barcode', $barcode)->count();
        $isMultiBrand = $brandCount > 1;
        ...
        // 4. Trigger Message hanya jika Multi-Brand dan status BERUBAH
        if ($isMultiBrand && ($oldStatus !== $newStatus)) {
            $action = ($newStatus === 1) ? 'DEACTIVATED' : 'REACTIVATED';
            $this->sendRobotNotification($mbMaster, $action);
        }
    ...
    private function sendRobotNotification($mbMaster, $action) {
        ...
        $jsonArray = [
            ...
            'text' => [
                ...
                'content'  => $this->messageCreation($mbMaster, $action),
                'hideAvatar' => '0',
                'btnOrientation' => '0',
                'btns' => [
                    [
    ...
    private function messageCreation($mbMaster, $action)
    {
        ...
        $recapMessage = "{$headerEmoji} *Hi MB STATUS CHANGE REPORT*\n";
        $recapMessage .= "------------------------------------------\n";
        ...
        $recapMessage .= "• Status : {$statusLabel}\n\n";

        $recapMessage .= "⚠️ _Catatan: Barcode ini merupakan bagian dari Multi-Brand._\n";
        $recapMessage .= "------------------------------------------\n";
        $recapMessage .= "🤖 _Auto-generated by System_";
